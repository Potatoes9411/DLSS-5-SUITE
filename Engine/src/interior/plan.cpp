...
{
    static constexpr auto ContainsWidth = [] [[nodiscard]] (const QualityRange& range, const Extent& input) noexcept -> bool {
...
{
    static constexpr auto IsAutoUpscale = [] [[nodiscard]] (const Options& options, const Extent& source, const Extent& target) noexcept -> bool {
        static constexpr auto IsSameExtent = [] [[nodiscard]] (const Extent& a, const Extent& b) noexcept -> bool { return a == b; };
...
{
    static constexpr auto Ratio = [] [[nodiscard]] (const Extent& input, const Extent& output) noexcept -> float {
        return static_cast<float>(output.width.Get()) / static_cast<float>(input.width.Get());
    };

    static constexpr auto OrderFor = [] [[nodiscard]] (float ratio) noexcept -> const QualityOrder& {
        static constexpr auto OrderAboveNative = [] [[nodiscard]] (float ratio) noexcept -> const QualityOrder& {
            static constexpr auto OrderAboveQuality = [] [[nodiscard]] (float ratio) noexcept -> const QualityOrder& {
                static constexpr auto OrderAbovePerformance = [] [[nodiscard]] (float ratio) noexcept -> const QualityOrder& { return ratio <= 2.4f ? kPerformanceOrder : kUltraOrder; };
    ...
    static constexpr auto Fits = [] [[nodiscard]] (const QualityTable& table, SrQuality quality, const Extent& input) noexcept -> bool {
        static constexpr auto RangeOf = [] [[nodiscard]] (const QualityTable& table, SrQuality quality) noexcept -> std::optional<QualityRange> {
...
{
    static constexpr auto ChooseSuperResolution = [] [[nodiscard]] (const Options& options, const Geometry& geometry,
                                                                    const QualityTable& table) noexcept -> Result<std::optional<SrChoice>, PlanError> {
        static constexpr auto RequiredSuperResolution = [] [[nodiscard]] (const Options& options, const Geometry& geometry,
                                                                          const QualityTable& table) noexcept -> Result<std::optional<SrChoice>, PlanError> {
    ...
    static constexpr auto WorkExtent = [] [[nodiscard]] (const std::optional<SrChoice>& sr, const Extent& source) noexcept -> Extent { return sr.has_value() ? sr->output : source; };

    static constexpr auto WithScales = [] [[nodiscard]] (const Options& o, const Geometry& g, const std::optional<SrChoice>& sr, const Extent& work,
                                                         const Extent& flow) noexcept -> Result<SessionPlan, PlanError> {
        static constexpr auto ScaleBetween = [] [[nodiscard]] (PixelCount work, PixelCount source) noexcept -> Result<Scale, PlanError> {
        ...
        static constexpr auto Assemble = [] [[nodiscard]] (const Options& o, const Geometry& g, const std::optional<SrChoice>& sr, const Extent& work, const Extent& flow, Scale scaleX,
                                                           Scale scaleY) noexcept -> SessionPlan {
            static constexpr auto WantsCursor = [] [[nodiscard]] (const Options& options, const Geometry& geometry) noexcept -> bool {
            ...
            static constexpr auto ClampedFinest = [] [[nodiscard]] (LevelIndex requested, LevelCount levels) noexcept -> LevelIndex {
            ...
            static constexpr auto ChosenScale = [] [[nodiscard]] (const std::optional<MotionScale>& asked, Scale ratio) noexcept -> MotionScale { return asked.value_or(MotionScaleTag::Of(ratio)); };
