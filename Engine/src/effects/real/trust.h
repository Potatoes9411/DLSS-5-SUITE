...
// An approved file held open so it stays the file that was checked: the handle is shared for reading only,
// so nothing may write to it, delete it or rename it while the session holds it.
struct TrustedFile
{
    UniqueHandle handle;
    ...
    bool suitePinned; // the exact SUITE compatibility model digest, rather than an NVIDIA signature
};
...
// The one neural-rendering compatibility model shipped by SUITE may instead match its exact pinned SHA-256.
// Every other file verifies every Authenticode signature it carries, the first and each one after it, and fails
...
[[nodiscard]] infra::Result<TrustedFile, Error> OpenApproved(const interior::FilePath& path, ModelKind kind) noexcept;
